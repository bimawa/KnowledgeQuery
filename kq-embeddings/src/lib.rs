...
impl Embeddings {
    fn forward(&self, input_ids: &Tensor, token_type_ids: &Tensor) -> CResult<Tensor> {
        ...
        let word_emb = self
            .word_embeddings
            .index_select(&input_ids_flat, 0)?
            .reshape((input_ids.dim(0)?, seq_len, ()))?;
        ...
        let token_type_emb = self
            ...
            .reshape((token_type_ids.dim(0)?, seq_len, ()))?;
...
impl EmbeddingModel {
    ...
    pub fn load(&mut self) -> Result<()> {
        ...
        let model = self
            .load_sentence_transformer(vb, &st_config)
            .map_err(ce)?;
    ...
    pub fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let attention_mask = Tensor::new(attention_mask, &self.device)
            ...
            .map_err(ce)?;
        let token_type_ids =
            Tensor::zeros(input_ids.shape(), DType::I64, &self.device).map_err(ce)?;
    ...
    fn load_sentence_transformer(
        ...
        let position_embeddings = vb
            .get(
                (max_pos, bert_config.hidden_size),
        ...
        let ln_weight = vb
            .get(bert_config.hidden_size, "embeddings.LayerNorm.weight")?
            .to_dtype(DType::F32)?;
        let ln_bias = vb
            .get(bert_config.hidden_size, "embeddings.LayerNorm.bias")?
            .to_dtype(DType::F32)?;
        ...
        for i in 0..bert_config.num_hidden_layers {
            ...
            let ffn_dense = Linear::new(
                vb.get(
                    (bert_config.intermediate_size, bert_config.hidden_size),
                    &ffn_w1,
                )?,
            ...
            let ffn_out = Linear::new(
                vb.get(
                    (bert_config.hidden_size, bert_config.intermediate_size),
                    &ffn_w2,
                )?,
            ...
            attention_layers.push(EncoderLayer {
                ...
                ffn_layer_norm: LayerNorm {
                    weight: vb
                        .get(bert_config.hidden_size, &ffn_ln_w)?
                        .to_dtype(DType::F32)?,
                    bias: vb
                        .get(bert_config.hidden_size, &ffn_ln_b)?
                        .to_dtype(DType::F32)?,
                    eps: bert_config.layer_norm_eps,
                },
            });
...
impl SentenceTransformer {
    fn forward(
        ...
        let extended_mask =
            ((extended_mask.ones_like()? - &extended_mask)? * f64::MIN)?;
