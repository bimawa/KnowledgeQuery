...
pub fn register_vec_on_connection(db: &Connection) -> Result<()> {
    ...
    unsafe {
        type Fn = unsafe extern "C" fn(*mut rusqlite::ffi::sqlite3, *mut *mut std::os::raw::c_char, *const std::ffi::c_void) -> i32;
...
pub fn store_embedding(db: &Connection, file_id: i64, chunk_index: usize, chunk_text: &str, embedding: &[f32]) -> Result<()> {
    ...
    db.execute("INSERT INTO vec_chunks (rowid, file_id, chunk_index, chunk_text, content_hash) VALUES (?1, ?2, ?3, ?4, '')",
...
pub fn search_vector(db: &Connection, query_vector: &[f32], limit: usize) -> Result<Vec<SearchResult>> {
    ...
    Ok(stmt.query_map(rusqlite::params![f32_slice_to_bytes(query_vector), limit as i64], |row| {
        Ok(SearchResult { file_id: 0, path: String::new(), content: row.get(2)?, score: row.get::<_, f64>(1)? })
    })?.filter_map(|r| r.ok()).collect())
}

pub fn hybrid_search(db: &Connection, query: &str, query_vector: &[f32], fts_limit: usize, vector_limit: usize) -> Result<Vec<SearchResult>> {
    let fts = crate::search::search_fts(db, query, fts_limit)?.into_iter().map(|r| SearchResult { file_id: 0, path: r.path, content: r.context, score: r.score }).collect::<Vec<_>>();
    let vec = search_vector(db, query_vector, vector_limit)?;
    let mut merged: HashMap<String, SearchResult> = HashMap::new();
    for (w, results) in [(0.6f64, fts), (0.4, vec)] {
        for r in results { merged.entry(r.path.clone()).and_modify(|e| e.score = e.score.max(r.score * w)).or_insert(SearchResult { score: r.score * w, ..r }); }
    }
...
mod tests {
    ...
    fn test_vec_chunks_table() {
        ...
        conn.execute("CREATE TABLE files(id INTEGER PRIMARY KEY,path TEXT,content_hash TEXT,last_modified TEXT,content TEXT)", []).ok();
        let _ = init_vector(&conn);
        let ok: bool = conn.query_row("SELECT COUNT(*) > 0 FROM sqlite_master WHERE type='table' AND name='vec_chunks'", [], |r| r.get(0)).unwrap_or(false);
        assert!(ok);
    }
