...
fn amend_commit_with_readme(repo: &Repository, dir: &Path) -> Result<git2::Oid> {
...
fn commit_without_readme(repo: &Repository, dir: &Path) -> Result<Option<git2::Oid>> {
    let mut index = repo.index().context("Failed to open index")?;
    index
...
pub fn push(path: &Path, dry_run: bool, no_readme: bool) -> Result<()> {
    ...
    if let Err(e) = fetch_result {
        if e.class() == git2::ErrorClass::Rebase
    ...
    if dry_run {
        ...
        eprintln!(
            ...
            head_commit.summary().unwrap_or("")
        );
        eprintln!(
            "  remote: {} {}",
            &remote_commit.id().to_string()[..8],
            remote_commit.summary().unwrap_or("")
        );
        return Ok(());
    }
    ...
    let oid = if !no_readme {
        ...
        amend_commit_with_readme(&repo, path)?
    } else {
        match commit_without_readme(&repo, path)? {
            Some(oid) => oid,
            None => {
...
mod tests {
    ...
    fn test_commit_without_readme() {
        ...
        let result = commit_without_readme(&repo, dir.path()).unwrap();
